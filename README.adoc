...
It uses http://d3js.org[D3] as its charting toolkit and tries to simplify the creation of advanced metrics visualizations with Angular.js.

== About

Hawkular Charts is a direct result of needing metric visualizations for the parent project:
...
This will also make adhoc development quicker/easier with components that can render Hawkular Metrics charts with very little effort.
...
* Run the `bower link` command from inside hawkular-charts

* From Hawkular project build with `mvn clean install -Pdev,link -Dbower.link.package=hawkular-charts`
